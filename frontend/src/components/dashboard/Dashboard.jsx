...
const Dashboard = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Set Plots {we are loaging react frontend but our image inside djano bcked so here how we load image from djanjo so django url is http://127.0.0.1:8000/ but react is i at 5173 port}
            setPlot(plotUrl)
            setMA100(ma100Url)
            setMA200(ma200Url)
        ...
        <div className='row'>
            <div className='col-md-6 mx-auto'>
                <form onSubmit={handleSubmit}>
                    ...
                    />
                    <small>{error && <div className='text-danger'>
                        {error}
                        </div>}</small>
                    <button type='Submit' className='btn btn-info mt-3'>
                        {loading ? <span>
                            <FontAwesomeIcon icon={faSpinner} spin /> Please Wait...
                        </span>: 'See Predictions'}
                    </button>
                </form>
                 </div> 

                 {/*printing prediction plots */}
                 {prediction && (
